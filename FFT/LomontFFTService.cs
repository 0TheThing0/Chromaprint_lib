...
public class LomontFFTService : IFFTService
{
    ...
    public void Initialize(int frame_size, double[] window)
    {
        ...
        _frame_size_h = frame_size / 2;
        _window = window;

        _input = new double[frame_size];
    ...
    public void ComputeFrame(short[] input, double[] output)
    {
        ...
        {
            _input[i] = (double)input[i];
        }

        Helper.ApplyWindow(ref _input, _window, _frame_size, 1.0);
        ...
        // Compute energy
        output[0] = _input[0] * _input[0];
        output[_frame_size_h] = _input[1] * _input[1];

        for (int i = 1; i < _frame_size_h; i++)
        {
            output[i] = _input[2 * i] * _input[2 * i] + _input[2 * i + 1] * _input[2 * i + 1];
        }
    }

    private void ComputeTable(int size)
    {
